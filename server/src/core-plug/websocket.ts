...
const encodeWsFrame = (data) => {
  ...
  frame = payloadData
    ? Buffer.concat([Buffer.from(frame), payloadData] as any)
    : Buffer.from(frame);
  return frame;
};
...
const websocket: Plugin = function ({ url, headers, socket }) {
  ...
  const reg =
    Object.prototype.toString.call(this.options.websocketUrl) ===
    "[object RegExp]"
        ...
        if (this.options.debug) {
          ncol.color(function () {
            this.log(`【websocket】：`).info(
              `websocket版本->sec-websocket-version：${headers["sec-websocket-version"]}`
            );
          });
          ncol.color(function () {
            this.log(`【websocket】：`).info(
              `websocket连接成功->客户端密钥：${key}`
            );
          });
        }
        // 注册监听
        if (
          Object.prototype.toString.call(this.options.websocket) ===
          "[object Object]"
        ) {
          Object.keys(this.options.websocket || {}).forEach((k) => {
            ...
            if (
              ...
              !(this.options.websocket[k] as any).isOn
            ) {
