import {
  ...
  createRoute as createRouteType,
  route,
  RouteOptionsRow,
} from "./types/type";
          ...
          _this.options.cors
            ? {
                "access-control-allow-origin":
                  _this.request.headers.origin || "*",
                "access-control-allow-methods": "*",
                "access-control-allow-headers":
                  _this.options.corsHeaders || "*",
              }
            : {},
...
export class createAppServe implements AppServe {
  ...
  async listen(port?: number): Promise<Server> {
    ...
    const listenPort =
      process.env.SERVE_PORT || typeof port === "number"
        ? port
        : this.options.serve.port;
    ...
    if (this.options.serve.LogServeInfo) {
      ...
      ncol.info(
        `ready in ${Math.ceil(performance.now() - global.__vite_start_time)}ms.`
      );
    }
    return this.Serve;
  }
  ...
  async init() {
    try {
      ...
      if (!!this.options.query_params && !route) {
        route = Object.values(this.RouteOptions).find(
          (e: any) => e.reg && e.reg.test(this.$url)
        ) as any;
      ...
      if (
        route &&
        types.includes(Object.prototype.toString.call(route.controller))
      ) {
        this.$route = route;
        ...
        const fn = async (p_route) => {
          //todo 判断请求方式
          const method: any =
            Object.prototype.toString.call(p_route.method) === "[object String]"
              ? [p_route.method]
              : Object.prototype.toString.call(p_route.method) ===
                "[object Array]"
              ? p_route.method
              : [];
          const isValidMethod =
            method.length > 0
              ? method
                  ...
                  .indexOf(this.request.method.toLowerCase()) > -1
              : true;
          if (!isValidMethod) {
          ...
          //todo 控制器兼容执行
          if (
            types.includes(Object.prototype.toString.call(p_route.controller))
          ) {
            const controllerName = p_route.funName || "default";
            try {
              if (
                Object.prototype.toString.call(p_route.controller) ===
                "[object Promise]"
              ) {
                if (
                  Object.prototype.toString.call(await p_route.controller) ===
                  "[object Object]"
                ) {
                  p_route.controller =
                    (await p_route.controller)?.[controllerName] ||
                    p_route.controller;
                }
              }

              let result: any = await p_route.controller.call(
                this,
                this.request,
                this.response,
                resultMap
              ...
              try {
                ...
                const awaitResult = await defaultController.call(
                  this,
                  this.request,
                  this.response,
                  resultMap
...
export const createApp: createAppType = (
  options?: Partial<AppServeOptions>
) => {
